        ...
        <div class="col-md-12">
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <?php 
                    $sn=1;
                    $user_qry = "SELECT posts.postno, posts.post, posts.topic, posts.sn, posts.time, posts.email, state.id, state.name FROM posts LEFT JOIN state ON posts.email=state.email ORDER BY posts.postno";
                    $user_res = mysqli_query($con, $user_qry);
                    while($user_data = mysqli_fetch_assoc($user_res)){
                        ?>
                        <tr>
                            <td><?php echo $user_data['postno']; ?></td>
